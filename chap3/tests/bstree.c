...
static void
traverse (const struct bstree_n *node)
{
  if (node == NULL)
    return;
  traverse (node->left);
...
static void
insert_helper (bstree *tree, int num, int max)
{
  srand (time (NULL));
  ...
  for (int i = 0; i < num; i++)
    {
      ...
      if (bstree_insert (tree, node) != node)
        {              /* duplicate nodes */
...
static int
comp (const struct bstree_n *node, const struct bstree_n *other)
{
