 ...
 * number to mean that `node` is greater than `other`, and zero to mean that
 * the two nodes are equal. the meaning of `node` here is the same as that of
 * `node` in the find, insert, and remove functions, but `other` is the
 * intermediate variable passed by these functions, so just focus on `node`.
 */
