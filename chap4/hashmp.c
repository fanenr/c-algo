...
hashmp *
hashmp_init (hashmp *map, int type)
{
  map->head = NULL;
  map->ktype = type;
...
hashmp *
hashmp_reserve (hashmp *map, size_t cap)
{
  if (cap <= map->capacity)
    return map;
...
long
hashmp_hash (hashmp *map, union hashmp_k key)
{
  if (map->capacity == 0)
    return -1;
...
struct hashmp_p *
hashmp_find (hashmp *map, union hashmp_k key)
{
  int type = map->ktype;
...
struct hashmp_p *
hashmp_find_next (hashmp *restrict map, struct hashmp_p *restrict pair)
{
  if (pair == NULL)
    return NULL;
...
struct hashmp_p *
hashmp_insert (hashmp *restrict map, struct hashmp_p *restrict pair)
{
  size_t cap = map->capacity;

  if (cap * HASHMP_LOAD_FACTOR < map->size + 1)
    {               /* expand sapce */
...
void
hashmp_remove (hashmp *restrict map, struct hashmp_p *restrict pair)
{
