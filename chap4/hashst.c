...
hashst *
hashst_init (hashst *set, int type)
{
  set->head = NULL;
  set->vtype = type;
...
extern hashst *
hashst_reserve (hashst *set, size_t cap)
{
  if (cap <= set->capacity)
    return set;
...
static size_t
hashst_hash_sdbm (const char *str)
{
  if (str == NULL)
    return 0;
  size_t hash = 0;
...
long
hashst_hash (hashst *set, union hashst_v val)
{
  if (set->capacity == 0)
    return -1;
...
union hashst_v *
hashst_find (hashst *set, union hashst_v val)
{
  int type = set->vtype;
...
union hashst_v *
hashst_insert (hashst *set, union hashst_v val)
{
  size_t cap = set->capacity;

  if (cap * HASHST_LOAD_FACTOR < set->size + 1)
    {               /* expand sapce */
...
void
hashst_remove (hashst *set, union hashst_v val)
{
